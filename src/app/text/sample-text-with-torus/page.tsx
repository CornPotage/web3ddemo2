...
// https://codesandbox.io/s/05-demo--typing-effects-with-webgl-tutorial-rtrk5f?file=/src/index.js:3278-3284
...
const TextCanvas = () => {
    ...
    const renderText = (textureCanvas: HTMLCanvasElement) => {
        ...
        const textureFont = `100 ${textureFontSize}px ${textureFontName}`
        const lines = [
            'ステーキハウスほし',
            'ドーナッツ食いたい',
        ]
    ...
    useEffect(() => {
        ...
        const geometry = new THREE.TorusGeometry(0.35, 0.15, 16, 50)
        ...
        coordinates.forEach((coordinate, index) => {
            tempThreeObject.rotation.set(2 * chance.floating({min: 0, max: 1}), 2 * chance.floating({min: 0, max: 1}), 2 * chance.floating({min: 0, max: 1}))
            tempThreeObject.position.set(
                coordinate.x + chance.floating({min: -2, max: 2}),
                (sceneHeight - coordinate.y) + chance.floating({min: -2, max: 2}),
                chance.floating({min: -2, max: 2})
            )
            tempThreeObject.updateMatrix()
            mesh.setMatrixAt(index, tempThreeObject.matrix)
        })
